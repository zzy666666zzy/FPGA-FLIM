function y = Generate_decay_tri_exp(tau1,tau2,tau3,alpha,beta,gamma,Npeak,FWHM)
...
tau2=tau2/h;  % ifetime 2 (ns)  
tau3=tau3/h; % ifetime 3 (ns)  
...
%lifetime
 %decay curve
f=alpha*exp(-t/tau1)+beta*exp(-t/tau2)+gamma*exp(-t/tau3); 
% measured decay
    C=conv(I,f);
    y =C(1:length(t));
    y =y /max(y);  
